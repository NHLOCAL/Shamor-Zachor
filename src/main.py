...
def main(page: Page):
    ...
    def create_tracking_page():
        # ... (implementation as before) ...
        nonlocal current_tracking_segment
        in_progress_items = []; completed_items = []
        for category, masechtot in data.items():
            for masechta_name, masechta_data in masechtot.items():
                progress = ProgressManager.load_progress(page, masechta_name, category); is_complete = is_masechta_completed_helper(category, masechta_name)
                if not progress and not is_complete: continue
                total_pages = get_total_pages(masechta_data); completed_pages = get_completed_pages(progress, ["learn"]); percentage = 0
                ...
                completion_date_str = ProgressManager.get_completion_date(page, masechta_name, category); hebrew_date_str = get_completion_date_string(completion_date_str) if completion_date_str else None
